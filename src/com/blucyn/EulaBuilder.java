...
public class EulaBuilder {
    ...
    // Eula's current stats.
    // (no artifacts)
    /** DON'T CHANGE THESE !!!! **/
    public double BASE_ATK           = 342;
    public double BASE_CRIT_DMG      = .884;
    public double BASE_CRIT_RATE     = .05;
    public double BASE_PHYSICAL_BONUS = 0;
    public double BASE_ENERGY        = 1;
    public double BASE_BONUS_DAMAGE = 1.00;

    public boolean chongyun = false;
    ...
    public EulaBuilder(ArrayList<Artifact> allArtifacts){
    ...
    public double getDamage(double atk, double elementalBonus){
        ...
        return 1.774 * atk * (1 + elementalBonus);
    ...
    public double getDamageBurst(double atk, double elementalBonus){
        ...
        double scaling = 7.256 + (stacks * 1.482);
        return scaling * atk * (1 + elementalBonus);
    ...
    public ArrayList<int[]> getBestCombos(boolean critBuild, boolean optimizeBurst){
                    ...
                    for (int gobletCount = 0; gobletCount < goblets.size(); gobletCount++) {
                        for (int circletCount = 0; circletCount < circlets.size(); circletCount++) {
                            for (int weapon = 0; weapon < weapons.size(); weapon++) {
                                ...
                                double[] damageValues = getArtComboDamage(comboCode, false, optimizeBurst);
    ...
    public double[] getArtComboDamage(int[] comboCode, boolean print, boolean optimizeBurst){
        ...
        if(print)
        {
            ...
            System.out.println("Slot\t\tATK\t\tATK%\tCRIT RATE\tCRIT DMG\tEnergy\t\tPhys Bonus\t\tSet Name");
            ...
            for(Artifact artifact : artifacts)
            {
                ...
                System.out.print("\t" + padString(Math.round(artifact.getStat("PHYSICAL_DMG_BONUS" ) * 10000) / 100.0 + "%", " ", 8));
        ...
        double newPhysBonus = BASE_PHYSICAL_BONUS + getStatSum(artifacts, "PHYSICAL_DMG_BONUS");
        double newCryoBonus = getStatSum(artifacts, "CRYO_DMG_BONUS");
        double newEnergy = BASE_ENERGY + getStatSum(artifacts, "ENERGY_RECHARGE");
        double additionalDamage = BASE_BONUS_DAMAGE;
        ...
        if(setBonuses.contains("2:BLOODSTAINED_CHIVALRY"))
        {
            newPhysBonus += .25;
        }
        if(setBonuses.contains("2:PALE_FLAME"))
        {
            newPhysBonus += .25;
        }
        ...
        double damage = optimizeBurst ?
                getDamageBurst(newAtk, newCryoBonus):
                chongyun ?
                        getDamage(newAtk, newCryoBonus):
                        getDamage(newAtk, newPhysBonus);
        ...
        if(print)
        {
            ...
            System.out.println("ATK\t\t\tCRIT RATE\tCRIT DMG\tEnergy\t\tPhys Bonus");
            ...
            System.out.print("\t" + padString(Math.round(newPhysBonus   * 10000) / 100.0 + "%" + "", " ", 8));
            System.out.println("\n");
            System.out.print(optimizeBurst ? "Burst" : "Normal");
    ...
    public void         askFood(){
        ...
        if(answer.equals("Y"))
        {
            ...
            System.out.print("[Builder]: " + "Will you be using a tianshu meat? (y/n): ");
            answer = s.nextLine().toUpperCase();
            if(answer.equals("Y"))
            {

                BASE_PHYSICAL_BONUS += .25;
                BASE_CRIT_RATE += .08;
    ...
    public void       askWeapon() {
        ...
        System.out.println("1 --> Wolf's Gravestone");
        System.out.println("2 --> Luxurious Sea-Lord");
        System.out.println("3 --> Favonius");
    ...
    public void     askChongyun() {

        System.out.print("[Builder]: " + "Will you be attacking within a chongyun field? (y/n): ");
    ...
    public void scrollBuilds(boolean critBuild, boolean optimizeBurst){

        ArrayList<int[]> comboCodes = getBestCombos(critBuild, optimizeBurst);
        if(highest_damage == null)
        {
            highest_damage = getArtComboDamage(comboCodes.get(0), false, optimizeBurst);
        }
        ...
        for(int i = 0; i < comboCodes.size(); i++)
        {
            ...
            double[] damage = getArtComboDamage(comboCode, true, optimizeBurst);
            double[] damageOpposite = getArtComboDamage(comboCode, false, !optimizeBurst);
            System.out.println();
            System.out.print(optimizeBurst ? "Normal" : "Burst");
            System.out.println(" Attack:");
            System.out.println("Average Hit Damage: " + damageOpposite[0]);
            System.out.println("   Crit Hit Damage: " + damageOpposite[1]);
            System.out.println();
    ...
    public void showBestBuilds(boolean askExtraQuestions){
        ...
        wipeScreen();
        boolean optimizeBurst = askBurst();
        wipeScreen();
        ...
        while(true)
        {
            scrollBuilds(critBuild, optimizeBurst);
        }
